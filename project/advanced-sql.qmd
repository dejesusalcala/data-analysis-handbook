...
First, let's use `engine.connect()` to perform a SQL query. Inside of `engine.conntect()` we can pass a SQL query in the form of a string

```{python}
# Query first 5 records in the the Database
with engine.connect() as connection:
    # Append LIMIT directly to your SQL string
    query = text("SELECT * FROM Census_Data LIMIT 5")
...
query_data = text("SELECT * FROM Census_Data")
...
A good way to think about these commands is to think of them in the following way 

- `text(...)` $\rightarrow$ tells SQLAlchemy "this is a SQL statement"
- `engine.connect()` $\rightarrow$ opens a database connection
- `with` $\rightarrow$ makes sure that connection gets cleaned up properly
